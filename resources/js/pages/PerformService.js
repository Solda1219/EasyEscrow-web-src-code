...
const PerformService = () => {
    ...
    const handelExchange = async () => {
        ...
        let myEscrows = escrows.filter(
            (escrow) => escrow.from == ctx.auth.actor && typeId == 4
        );
        let loopId = 2;
        if (!myEscrows.length) {
        ...
        <Fragment>
            <main>
                <div className="container banner-inner">
                    <h1>Perform a Service</h1>
                    <h3>
                        This escrow allows any user to easily utilize a smart
                        contract to verify and pay for any service with select
                        cryptocurrencies
                    </h3>
                </div>
                    ...
                    <form onSubmit={handlePurchaseFormSubmit}>
                        <div className="form-row">
                            <label htmlFor="purchase-title">
                                Add a title for the service to purchase
                            </label>
                            <input
                                type="text"
                                id="purchase-title"
